...
const SignupInput = React.forwardRef<TextInput, SignupInputProps>(
  (
    {
      label,
      error,
    ...
    return (
      <View className={["mb-4", containerClassName].filter(Boolean).join(" ")}>
        {label && (
          <Text
            className={[
              "text-text-DEFAULT mb-2 font-pretendard-semibold text-[16px] tracking-[0px]",
              labelClassName,
            ]
              .filter(Boolean)
              .join(" ")}
        ...
        <View className="mt-2 h-[28px] flex-row items-start justify-between">
          <Text
            className={[
              "flex-1 font-pretendard text-caption",
          ...
          {maxLength && (
            <Text
              className={[
                "w-[40px] text-right font-pretendard text-caption text-text-subdued",
                counterClassName,
              ]
                .filter(Boolean)
                .join(" ")}
