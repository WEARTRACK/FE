...
export function ClosetStatsScreen() {
  ...
  return (
    <View className="flex-1 bg-bg-light px-6">
      ...
      <ScrollView
        contentContainerStyle={{ paddingTop: summaryTop, paddingBottom: 40 }}
              ...
              <Svg width={DONUT_SIZE} height={DONUT_SIZE}>
                ...
                <G rotation={-90} originX={DONUT_SIZE / 2} originY={DONUT_SIZE / 2}>
                  {chartData.map((target, index) => {
                    const segmentLength =
                      totalValue === 0 ? 0 : (target.value / totalValue) * circumference;
                ...
                {statistics.rankedCategories.map((target) => (
                  ...
                  >
                    <View className="flex-row items-center">
                      ...
                      />
                      <Text className="ml-[13px] font-pretendard text-body text-text">
                        {target.label}
                      </Text>
                    </View>
